<form method="POST" action="{{ route('admin/list/save', ['id' => $list->id ?: 0 ]) }}">
    ...
    <div class="form-group row">
        <label for="descriptiondescription{{ $list->id ?: 0 }}" class="col-md-2 col-form-label text-md-right">{{ __('Description') }}</label>
        <div class="col-md-8">
            <textarea id="descriptiondescription{{ $list->id ?: 0 }}" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" name="description" >{{ old('description', $list->description ?: null) }}</textarea>
...
<script>
    jQuery('document').ready(function(){
        $("#descriptiondescription{{ $list->id ?: 0 }}").summernote({
